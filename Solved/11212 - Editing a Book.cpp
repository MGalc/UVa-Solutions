// Double ended BFS
// - One from starting position
// - One from the final position
//
// Runs in 0.220 seconds (compared to 7.5 seconds of normal BFS)
...
int main() {
    ...
    while (cin >> N, N) {
        ...
        while (!q.empty()) {
            ...
            int dist = dists[m];
            if (dist >= 3) break; // TODO: Possible change to 3
        ...
        while (!q.empty()) {
            ...
            for (int i = 0; i < N; i++) {
                for (int j = i; j < N; j++) {
                    ...
                    for (int k = num-1; k < N; k++) {
                        ll perm = moveFront(shifted, num, k);
                        int permdist = dists[perm];
                        if (permdist == 0) {
                            dists[perm] = dist-1;
                            q.push(perm);
                        }
                        else if (permdist > 0) {
                            dists[SOL_VALUE] = 1 + permdist - dist;
                            goto end;
                        }
                    }
                }
